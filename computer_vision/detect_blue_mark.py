...
def main(a):
    _point = [0,0,0,0,0]
    #robot_set_com_queue = Queue()
    #robot_set_event = threading.Event()
    #robot_set = RobotArm(robot_set_com_queue, robot_set_event)
    #robot_set.start()
    # Create mouse event
    if a == 1: #`kato commentout`
        cv2.namedWindow("Color frame") #comment out kato 0201
        cv2.setMouseCallback("Color frame", show_distance) #comment out kato 0201
    
    
    
    #while True:#kato comment out
    ...
    #robot_set.connect()
    #pos = [ 0.04162088, 0.96691024, -1.98562271, 1.20379448, 0.88048851]
    #robot_set.set_arm_position(pos)
        # cv2.imshow("HSV Color frame", hsv_color_img)
        # cv2.imshow("Output frame", output_img)
        # cv2.imshow("mask", mask)
    #key = cv2.waitKey(1)#1-0
        # break
    #if key == 27:
        #break

    return _point #kato edit